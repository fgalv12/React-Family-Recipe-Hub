...
describe('Family Recipe Hub App', () => {
  ...
  test('displays the RecipeForm when Add is clicked', () => {
    ...
    expect(screen.getByPlaceholderText('Instructions (separate by periods)')).toBeInTheDocument();
  });
  ...
  test('resets the search and view when Home is clicked', () => {
    ...
    test('adds a new recipe when handleSaveRecipe is called', () => {
      ...
      fireEvent.change(screen.getByPlaceholderText('Recipe Name'), { target: { value: 'New Recipe' } });
      fireEvent.change(screen.getByPlaceholderText('Category'), { target: { value: 'Dessert' } });
      fireEvent.change(screen.getByPlaceholderText('Ingredients (separate by commas)'), { target: { value: 'Sugar, Flour, Butter' } });
      fireEvent.change(screen.getByPlaceholderText('Instructions (separate by periods)'), { target: { value: 'Mix ingredients. Bake for 30 minutes.' } });
  ...
  test('displays the RecipeForm with existing data when editing a recipe', () => {
    ...
    expect(screen.getByPlaceholderText('Instructions (separate by periods)').value).toContain('Boil water in a pot.');
  });
